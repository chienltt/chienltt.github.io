...
function Keycloak(config) {
  ...
  function generateRandomString(len, alphabet) {
    ...
    for (var _i = 0; _i < len; _i++) {
      chars[_i] = alphabet.charCodeAt(randomData[_i] % alphabet.length);
    }
  ...
  function generatePkceChallenge(pkceMethod, codeVerifier) {
    switch (pkceMethod) {
      case "S256":
        var hashBytes = new Uint8Array(_jsSha["default"].arrayBuffer(codeVerifier));
        var encodedHash = _base64Js["default"].fromByteArray(hashBytes).replace(/\+/g, '-').replace(/\//g, '_').replace(/\=/g, '');
        return encodedHash;
      default:
        throw 'Invalid value for pkceMethod';
    }
          ...
          req.onreadystatechange = function () {
            if (req.readyState === 4) {
              if (req.status === 200) {
                ...
                for (var _p = refreshQueue.pop(); _p != null; _p = refreshQueue.pop()) {
                  _p.setError(true);
                }
              }
            }
    ...
    if (configUrl) {
      ...
      req.onreadystatechange = function () {
        if (req.readyState === 4) {
          if (req.status === 200 || fileLoaded(req)) {
            var _config = JSON.parse(req.responseText);
            kc.authServerUrl = _config['auth-server-url'];
            kc.realm = _config['realm'];
            kc.clientId = _config['resource'];
      ...
      if (!oidcProvider) {
        if (!config['url']) {
          ...
          for (var _i2 = 0; _i2 < _scripts.length; _i2++) {
            if (_scripts[_i2].src.match(/.*keycloak\.js/)) {
              config.url = _scripts[_i2].src.substr(0, _scripts[_i2].src.indexOf('/js/keycloak.js'));
              break;
            }
          }
        }
        ...
        if (typeof oidcProvider === 'string') {
          ...
          _req.open('GET', oidcProviderConfigUrl, true);
          _req.setRequestHeader('Accept', 'application/json');
          _req.onreadystatechange = function () {
            if (_req.readyState === 4) {
              if (_req.status === 200 || fileLoaded(_req)) {
                var oidcProviderConfig = JSON.parse(_req.responseText);
  ...
  function parseCallbackParams(paramsString, supportedParams) {
    ...
    for (var _i3 = 0; _i3 < p.length; _i3++) {
      var split = p[_i3].indexOf("=");
      var key = p[_i3].slice(0, split);
      if (supportedParams.indexOf(key) !== -1) {
        result.oauthParams[key] = p[_i3].slice(split + 1);
      } else {
        if (result.paramsString !== '') {
          result.paramsString += '&';
        }
        result.paramsString += p[_i3];
      }
    }
    return result;
  }
  ...
  function setupCheckLoginIframe() {
    ...
    var messageCallback = function messageCallback(event) {
      ...
      for (var _i4 = callbacks.length - 1; _i4 >= 0; --_i4) {
        var _promise = callbacks[_i4];
        if (event.data === 'error') {
          _promise.setError();
        } else {
          _promise.setSuccess(event.data === 'unchanged');
        }
      }
    };
  ...
  var CookieStorage = function CookieStorage() {
    ...
    var getCookie = function getCookie(key) {
      ...
      for (var _i6 = 0; _i6 < ca.length; _i6++) {
        var c = ca[_i6];
        while (c.charAt(0) === ' ') {
          c = c.substring(1);
        }
